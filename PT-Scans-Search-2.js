function init() {
  $ui.register((ctx) => {
    ...
    /* fetchAniList: busca por username (fallback público) */
    ...
    /* fetchAniListWithToken: usa Bearer token OAuth para carregar biblioteca do utilizador autenticado */
    panel.channel.on("fetchAniListWithToken", async (token) => {
      ...
      try {
        ...
        const listRes = await fetch("https://graphql.anilist.co", {
          method: "POST", headers,
          body: JSON.stringify({
            query: "query { MediaListCollection(userId: " + viewer.id + ", type: MANGA, status: CURRENT) { lists { entries { progress media { id chapters title { romaji english } coverImage { large } } } } } }"
          })
        });
        const listData = await listRes.json();
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; margin:0; padding:0; }
    ...
    .ali-login-area { display:flex; align-items:center; gap:10px; flex-wrap:wrap; }
    .ali-sep { color:#5a6e8c; font-size:12px; font-weight:700; padding:0 4px; }
    .ali-fallback { display:flex; align-items:center; gap:8px; }
...
<body>
  ...
  <script>
    ...
    const state = {
      ...
      anilistToken:"", anilistViewer:null,
      providerModal:null
    };
    ...
    /* ── LIBRARY ── */
    /* OAuth AniList: abre popup, recebe token via postMessage */
    function openAniListLogin(clientId) {
      ...
      /* Poll popup location para capturar o token no hash */
        ...
        try {
          ...
          if (hash && hash.includes("access_token")) {
            ...
            if (token) {
              ...
              window.webview.send("fetchAniListWithToken", token);
            }
          }
    ...
    function renderLibrary(){
      ...
      /* ── Header com login/utilizador ── */
      if (state.anilistViewer) {
        const v = state.anilistViewer;
        const avatar = v.avatar ? '<img class="ali-avatar" src="' + esc(v.avatar) + '" alt="avatar" />' : '';
        html += '<div class="lib-header">' +
          '<div class="ali-user">' + avatar + '<span class="ali-name">' + esc(v.name) + '</span></div>' +
        ...
        html += '<div class="lib-header">' +
          '<div class="ali-login-area">' +
            '<input id="aniClientIdInput" class="lib-input" placeholder="Client ID AniList (opcional)" style="width:220px;" />' +
            '<button id="loginOAuthBtn" class="btn btn-anilist">🔐 Login com AniList</button>' +
          '</div>' +
          '<div class="ali-sep">ou</div>' +
          '<div class="ali-fallback">' +
            '<input id="libUserInput" class="lib-input" placeholder="Nome de utilizador" value="' + esc(state.libraryUser) + '" />' +
      ...
      if(state.loading){
        ...
        html+='<div class="empty"><div class="empty-box">'+
          ...
          (state.anilistViewer ? "Sem mangas na lista CURRENT." : "Faz login com AniList ou insere o teu nome de utilizador para carregar a lista de leitura.") +
          '</div>'+
        '</div></div>';
      }
      html+='</div>';
    ...
    function attachLibraryEvents(){
      ...
      if(loginOAuthBtn){
        loginOAuthBtn.addEventListener("click",()=>{
          const clientIdInput=document.getElementById("aniClientIdInput");
          const clientId=(clientIdInput&&clientIdInput.value.trim())||"23990";
          openAniListLogin(clientId);
        });
      }
      ...
      /* Recarregar biblioteca (com token guardado) */
    ...
    window.webview.on("anilistViewer", v=>{state.anilistViewer=v||null;render();});
