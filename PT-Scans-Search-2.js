function init() {
  $ui.register((ctx) => {
    ...
    /* ── AniList – fetch por username (público) ── */
    ...
    panel.channel.on("fetchAniListWithToken", async (token) => {
      ...
      loading.set(true); status.set("A carregar biblioteca...");
      try {
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; margin:0; padding:0; }
    ...
    /* ── Library tabs & AniList styles ── */
    .lib-tabs { display:flex; gap:0; margin-bottom:18px; border-radius:16px; overflow:hidden; border:1px solid rgba(255,255,255,.1); background:rgba(255,255,255,.03); flex-shrink:0; }
    ...
    .lib-oauth-box { display:flex; flex-direction:column; align-items:center; gap:14px; padding:28px 20px; background:rgba(2,169,255,.05); border-radius:20px; border:1px solid rgba(2,169,255,.15); }
    .lib-oauth-box p { font-size:12px; color:#7a94be; text-align:center; max-width:320px; line-height:1.6; margin:0; }
...
<body>
  ...
  <script>
    ...
    function renderLibrary(){
      ...
      /* painel oauth */
      html+='<div class="lib-tab-panel'+(state.libMode==="oauth"?" active":"")+'">';
      if(state.anilistViewer){
        ...
        html+='<div class="lib-oauth-box">'+
          ...
          '</div>'+
          '<button id="loginOAuthBtn" class="btn btn-anilist" style="width:100%;max-width:380px;height:48px;">🔐 Login com AniList</button>'+
          '<p style="font-size:11px;color:#4a6080;">Cria a app em: anilist.co → Perfil → Definições → Developer → New Client</p>'+
        '</div>'+
      ...
      /* painel username */
      html+='<div class="lib-tab-panel'+(state.libMode==="user"?" active":"")+'">';
      html+='<div class="lib-header">'+
    ...
    function attachLibraryEvents(){
      ...
      if(libInput) libInput.addEventListener("keydown",e=>{
        if(e.key==="Enter"){const u=e.target.value.trim();if(u){state.libraryUser=u;window.webview.send("fetchAniList",u);}}
      });
